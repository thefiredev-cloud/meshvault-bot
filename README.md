...
Default model path is **Qwen** (DashScope / compatible OpenAI API). OpenRouter and the rest of the Pi catalog stay available. Deployment-owned local models use the explicit `MESHVAULT_GATEWAY_*` OpenAI-compatible gateway.
...
- OpenRouter still works: `OPENROUTER_API_KEY`. For deployment-owned local models, set `MESHVAULT_GATEWAY_URL`, `MESHVAULT_GATEWAY_KEY`, and the exact comma-separated `MESHVAULT_GATEWAY_MODELS` served by that endpoint.
...
```bash
pnpm --filter @meshvault/db exec prisma migrate resolve --applied 0001_init
```
...
```bash
pnpm --filter @meshvault/desktop dev
```
...
Development defaults to `http://127.0.0.1:5173`. A packaged build asks for the HTTPS origin served by the Windows-owned MeshVault runtime and remembers it. Use **MeshVault → Change Server…** to change it; `MESHVAULT_WEB_URL` remains the explicit startup override.

Packaged installers (optional):

```bash
pnpm --filter @meshvault/desktop pack
```
...
pnpm --filter @meshvault/desktop release:mac
```
...
```bash
pnpm --filter @meshvault/mobile start
```
...
```bash
export MESHVAULT_IOS_BUNDLE_IDENTIFIER='<assigned bundle id>'
export MESHVAULT_IOS_BUILD_NUMBER='<next build number>'
export MESHVAULT_EXPO_OWNER='<Expo account>'
export MESHVAULT_EXPO_PROJECT_ID='<EAS project id>'
pnpm --filter @meshvault/mobile release:ios:check
...
`apps/www` is the public marketing site. It is not the signed-in product. Workspace packages use the `@meshvault/*` scope.
