...
describe("contracts", () => {
  it("parses bot create input", () => {
    ...
    expect(
      CreateBotInput.parse({ name: "Coder", modelProvider: "anthropic", modelId: "claude" }),
    ).toMatchObject({ modelProvider: "anthropic", modelId: "claude" });
    expect(() => CreateBotInput.parse({ name: "Broken", modelProvider: "anthropic" })).toThrow(
      /modelProvider and modelId must be set together/,
    );
  });
