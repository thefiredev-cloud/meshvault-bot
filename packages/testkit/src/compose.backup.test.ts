...
function postgresUp() {
  try {
    execSync(`docker compose -f ${composeFile} exec -T postgres pg_isready -U meshvault`, {
      stdio: "ignore",
      timeout: 8_000,
    });
    return true;
...
describeBackup("compose backup and restore", () => {
  it("dumps postgres and restores into a side database", () => {
    ...
    const dump = path.resolve("backups", stamp, "meshvault.sql");
    ...
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U meshvault -d postgres -c "DROP DATABASE IF EXISTS meshvault_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U meshvault -d postgres -c "CREATE DATABASE meshvault_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U meshvault -d meshvault_restore_test`,
      {
        input: sql,
    ...
    const tables = execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U meshvault -d meshvault_restore_test -c "\\dt"`,
      { encoding: "utf8", timeout: 20_000 },
    );
    expect(tables).toMatch(/bots/);
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U meshvault -d postgres -c "DROP DATABASE meshvault_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
