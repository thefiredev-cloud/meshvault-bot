# MeshVault — agent brief

Open-source Grok Bot alternative. Electron desktop + Expo iOS. BYO model. Default: Qwen (DashScope / OpenAI-compatible); deployment-owned local models use the explicit `MESHVAULT_GATEWAY_*` lane. Packages use `@meshvault/*`; upstream and license records live in `UPSTREAM.md`, `NOTICE`, and `LICENSE`.
